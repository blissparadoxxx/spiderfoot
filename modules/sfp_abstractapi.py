...
class sfp_abstractapi(SpiderFootPlugin):
    ...
    def handleEvent(self, event):
        """
        Handle an event received by the module.
        ...
        if eventName == "DOMAIN_NAME":
            self.handleDomainNameEvent(eventData, event)
        elif eventName == "PHONE_NUMBER":
            self.handlePhoneNumberEvent(eventData, event)
        elif eventName in ["IP_ADDRESS", "IPV6_ADDRESS"]:
            self.handleIpAddressEvent(eventData, event)

    def handleDomainNameEvent(self, eventData, event):
    ...
    def handlePhoneNumberEvent(self, eventData, event):
    ...
    def handleIpAddressEvent(self, eventData, event):
